  ...
  methods: {
    ...
    bodyDown (ev) {
      ...
      if (typeof ev.preventDefault !== 'undefined') {
        if (tagName !== 'INPUT' && tagName !== 'TEXTAREA' && tagName !== 'SELECT' && !ev.path.find(e => e.id === 'tipTap')) {
          ev.preventDefault()
        }
      }

      if (this.isDraggable && tagName !== 'INPUT' && tagName !== 'TEXTAREA' && tagName !== 'SELECT' && !ev.path.find(e => e.id === 'tipTap')) {
        this.bodyDrag = true
      }
    ...
    stickMove (delta) {
      ...
      // 좌표가 음수값 나오면 10으로 좌표 고정
      if (Math.sign(newLeft) === -1) {
        this.left = 10
      } else {
        this.left = newLeft
      }
