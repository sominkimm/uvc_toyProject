  ...
  <div>
    ...
    >
      <b-row class="uvc-module">
        ...
        >
          <b-form-select
            v-model="params.LsProtocol.memoryType"
        ...
        >
          <b-form-input
            v-model="params.LsProtocol.memoryAddr"
            type="text"
            required
          ...
          /><div class="app-description">
            {{ $t("profileMaker.description.memoryAddressInputDescriptionLS") }}
          </div>
          <div
            v-if="isSubmit && !$v.params.LsProtocol.memoryAddr.required"
        ...
        >
          <b-input-group class="input-wrap">
            <b-form-input
              v-model="params.LsProtocol.tagName"
              type="text"
              required
          ...
          <div
            v-if="isSubmit && !$v.params.LsProtocol.tagName.required"
            class="app-validation"
          >
      ...
      <b-card-title class="d-flex justify-content-between align-items-center">
        ...
        <div>
          ...
          <b-button-group
            ...
            size="sm"
          >
            <!-- @click="tagListEditStatus = !tagListEditStatus" -->
            <b-button
              dev-comment="수정완료/수정"
...
export default {
  ...
  methods: {
    ...
    checkEngNum (str) {
      const regExp = /^[a-zA-Z]{1}[0-9]{4}[a-zA-Z0-9]{1}$/g
      if (regExp.test(str)) {
        return true
      } else {
        return false
    ...
    newTagAdd () {
      ...
      const memoryAddrToUpper = this.params.LsProtocol.memoryAddr.toUpperCase()
      if (!this.$v.$invalid) { // validation 통과 시 submit 실행
        if (this.checkEngNum(this.params.LsProtocol.memoryAddr) &&
        this.typeSwitchFunc(this.params.LsProtocol.memoryAddr) === this.params.LsProtocol.memoryType) {
        // 태그 리스트의 태그명 중복 check
          if (this.selectedDevice.requests.find(e => e.name === this.params.LsProtocol.tagName)) {
          ...
          if (this.selectedDevice.requests.find(e => e.memoryAddr === memoryAddrToUpper)) {
          ...
          if (this.tagNameOverlapValidate === false && this.memoryAddrOverlapValidate === false) {
            ...
            this.selectedDevice.requests.push({
              ...
              name: this.params.LsProtocol.tagName,
              memoryType: this.params.LsProtocol.memoryType,
              memoryAddr: memoryAddrToUpper
            })
            // this.isSubmit = false
            this.onDataInit()
    ...
    checkDuplicates (originalArray, prop) {
      ...
      for (const arr of originalArray) {
        const isDuplicated = []
        for (const i in originalArray) {
          if (arr[prop] === originalArray[i][prop]) {
    ...
    typeSwitchFunc (address) {
      if (typeof address === 'string') {
        ...
        case 'P': case 'M': case 'L': case 'K': case 'F': case 'S': return 'BIT'
        case 'D': case 'U': case 'N': case 'Z': case 'T': case 'C': case 'R': return 'WORD'
        default: return null
        }
      } else {
        return null
    ...
    getResultRow (sheet) {
      ...
      const sheetDataAfterValidation = columnData.filter(p => p[memoryAddr] && p[name]) // 빈값 체크, memoryType 자동 생성
      const sheetData = sheetDataAfterValidation.map(p => {
        const randomA = Math.floor(Math.random() * 10000)
        const randomB = Math.floor(Math.random() * 10000)
        return {
          memoryType: this.typeSwitchFunc(p[memoryAddr], 'BIT', 'WORD'),
          memoryAddr: p[memoryAddr].toUpperCase(),
          name: p[name],
          tagId: +new Date() + randomA + randomB
        }
      })

      const rowAfterTypeCheck = sheetData.filter(p => p.memoryType) // 메모리 타입 빈값 체크
      // 빈값 필터, 중복 체크
      if (rowAfterTypeCheck.length === sheet.length) {
        const uniqueTagNameArray = this.checkDuplicates(rowAfterTypeCheck, 'name')
        const uniqueMemoryAddrArray = this.checkDuplicates(rowAfterTypeCheck, 'memoryAddr')
        ...
        if (uniqueTagNameArray.length === 0) { // 이름 중복
          if (uniqueMemoryAddrArray.length === 0) { // 메모리 번지 중복
            ...
            this.makeToastError({ code: 'FAIL', message: this.$t('modal.readDataFailDuplicateMemoryAddrLs'), status: 400 })
          }
        } else { // 이름 중복
